...
-- Additive fix: delayed subscription.create can link a verified non-renewing
-- subscription. Existing linked subscriptions remain protected against replay.
...
BEGIN
    ...
    IF p_provider_status IS NULL OR pg_catalog.btrim(p_provider_status) NOT IN ('active', 'non-renewing') THEN
        RAISE EXCEPTION 'Invalid provider status "%". Must be "active" or "non-renewing".', p_provider_status
            USING ERRCODE = '22023';
    END IF;
    v_provider_status := pg_catalog.btrim(p_provider_status);

    IF p_next_payment_at IS NULL AND v_provider_status = 'active' THEN
    ...
    IF FOUND THEN
        ...
        IF NOT FOUND
           ...
           OR (v_pm.authorization_signature <> v_authorization_signature AND v_pm.authorization_signature NOT LIKE 'SIG_AUTH_%')
        THEN
    ...
    FOR v_match_record IN
        ...
        WHERE pm.provider = 'paystack'
          ...
          AND p.purpose IN ('activation', 'renewal')
          AND p.auto_renew_requested IS TRUE
    ...
    IF v_provider_status = 'non-renewing'
       AND v_subscription.status NOT IN ('active', 'non_renewing', 'past_due') THEN
        RAISE EXCEPTION 'Cannot link non-renewing provider state to this subscription lifecycle.'
            USING ERRCODE = '23514';
    END IF;
    ...
    -- Align the first paid period to the verified provider calendar (including month-end dates).
    -- Do not let a delayed subscription.create overwrite a later renewed period.
    IF v_provider_status = 'active'
       AND p_next_payment_at IS NOT NULL AND p_next_payment_at > v_payment.paid_at
       AND v_subscription.current_period_start = v_payment.paid_at
       AND v_subscription.status = 'active' THEN
        UPDATE public.billing_subscriptions
        SET current_period_end = p_next_payment_at,
            next_renewal_at = p_next_payment_at,
            updated_at = v_now
        WHERE id = v_subscription.id;
    END IF;

    -- Current authenticated provider state wins over the original creation label.
    -- Preserve paid dates and any past-due state. No additional payment or access.
    IF v_provider_status = 'non-renewing' THEN
        UPDATE public.billing_subscriptions
